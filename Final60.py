...
class Eengezins():
    ...
    def positioner(self):
        ...
        if number == 0:
            #right wall
            self.y = randint(self.verplichte_vrijstand, HEIGHT-self.h_vr)
            self.x = (WIDTH-(self.w_vr + extraEengezins))
        elif number == 1:
            #left wall
            self.y = randint(self.verplichte_vrijstand, HEIGHT-self.h_vr)
            self.x = (self.verplichte_vrijstand+extraEengezins)
        elif number == 2:
            #bottom
            self.y = (HEIGHT-(self.h_vr+extraEengezins))
            self.x = randint(self.verplichte_vrijstand, WIDTH-self.w_vr)
        elif number == 3:
            #top
            self.y = (self.verplichte_vrijstand+extraEengezins)
            self.x = randint(self.verplichte_vrijstand, WIDTH-self.w_vr)
